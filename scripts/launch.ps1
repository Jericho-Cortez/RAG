...
if (Test-Path "$PROJECT_DIR\.env") {
    $envContent = Get-Content "$PROJECT_DIR\.env" -Raw
...
if ($groqKeyFound) {
    ...
    Write-Warn "Clé Groq non configurée → vérifie .env"
}
...
$cacheFile    = "$cacheDir\.corhack-cache-$VAULT_NAME.json"
$changedFiles = @()
...
& $VENV_PYTHON "$PROJECT_DIR\src\query.py"
